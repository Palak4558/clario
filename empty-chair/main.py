...
# Initialize clients once per function instance.
...
def get_embedding_vertexai(text_content):
    """Generate an embedding vector for a piece of text via Vertex AI."""
    if not text_content:
        return []
    try:
...
def startSession(request):
    """
    Initializes a new session. The user starts in a 'pre-analysis' phase
    where the AI guides them to articulate the core problem/emotion before
...
def analyzeInitialProblem(request):
    """
    Handles the pre-analysis dialogue phase. Guides the user until an analysis
    is explicitly requested (or the dialogue has gone on long enough), then
    identifies the core problem and transitions the session to the Empty
    Chair phase.
    """
    try:
        request_json = request.get_json(silent=True)
    ...
    try:
        ...
        print(f"ERROR retrieving conversation history for initial analysis: {e}")
    ...
    print(f"DEBUG: Message count: {len(conversation_history_for_ai)}, Explicit trigger: {explicit_trigger}, "
          f"Length trigger: {conversation_length_trigger}")

    ai_response_text = "An error occurred."
...
"""
            ...
            system_message = (
                f"You are an empathetic AI facilitator in a pre-analysis phase. The user wants to talk about "
                f"'{current_person_in_chair}' to achieve '{user_goal}'. Your goal is to help them articulate the "
                f"core issue by asking brief, insightful, open-ended questions (1-2 sentences). Do NOT provide "
                f"analysis or solutions. Just probe gently."
            )
            conversation_with_system = [Content(role="user", parts=[Part.from_text(system_message)])]
            conversation_with_system.extend(conversation_history_for_ai)

            try:
                response = model.generate_content(conversation_with_system)
                ai_response_text = response.text.strip()
            except Exception as e:
                print(f"ERROR in model generation with system message: {e}")
                response = model.generate_content(conversation_history_for_ai)
                ai_response_text = response.text.strip()

    except Exception as e:
        print(f"ERROR in analyzeInitialProblem model generation: {e}")
        ai_response_text = (
            f"I'm sorry, I'm having a technical issue. Could you please tell me more about what's on your mind "
            f"regarding '{current_person_in_chair}'?"
        )
    ...
    try:
        ...
        print(f"ERROR saving dialogue messages to Firestore: {e}")
...
def startEmptyChairSession(request):
    ...
    try:
        ...
        return (
            json.dumps({"error": "Missing userId or sessionId"}),
            400,
...
# --- processMessage (self-initializing) ---
@functions_framework.http
def processMessage(request):
    """
    Handles ongoing Empty Chair session messages, generates AI prompts, and
    manages conversation history in Firestore. Incorporates long-term memory
    from past sessions with the same 'personInChair' via semantic search
    (RAG over session summaries/embeddings). Auto-creates a session if one
    isn't supplied or found, so the client doesn't have to orchestrate
    startSession -> analyzeInitialProblem -> startEmptyChairSession every time.
    """
    try:
        request_json = request.get_json(silent=True)
    ...
    # --- Long-term memory RAG (same personInChair, most semantically relevant past sessions) ---
    ...
    try:
        ...
        for doc in past_sessions_query:
            ...
            if all(k in past_session_data for k in
                   ["blueSummary", "blueSummaryEmbedding", "redSummary", "redSummaryEmbedding",
        ...
        for score, sdata in candidates[:2]:
            relevant_summaries_text.append(
                f"Past Session ({sdata.get('startTime').strftime('%Y-%m-%d')}, goal: {sdata.get('userGoal')}):")
        ...
        if relevant_summaries_text:
            long_term_memory_context = (
                f"### User's Past Session Learnings (Semantic RAG for '{current_person_in_chair}'):\n"
                + "\n".join(relevant_summaries_text)
            )
    except Exception as e:
        print(f"ERROR fetching semantic long-term memory for RAG: {e}")
        long_term_memory_context = ""
    ...
    try:
        ...
        conversation_history.append(
            Content(role="user", parts=[Part.from_text(f"[{perspective.upper()} Chair]: {user_message_text}")])
        )
    except Exception as e:
        print(f"ERROR retrieving current conversation history: {e}")
    ...
    try:
        system_instruction_parts = [
            ...
            "You are guiding the user (who switches between 'BLUE Chair' and 'RED Chair') to gain clarity and insight. ",
            "Always respond with a short, open-ended question or prompt (1-2 sentences max) to encourage deeper "
            "reflection or to facilitate switching perspectives. ",
            "Help the user express themselves better, irrespective of the chair they are in. ",
            "Never offer advice, make statements, or answer questions directly. Just prompt. ",
            "Encourage switching perspectives when appropriate to explore new angles, but do not force it rigidly. ",
            long_term_memory_context + "\n" if long_term_memory_context else ""
        ]

        pre_analysis_root_emotion = session_details.get("preAnalysisRootEmotion", "Not identified")
        pre_analysis_cause_of_emotion = session_details.get("preAnalysisCauseOfEmotion", "Not identified")
        pre_analysis_statement = session_details.get("preAnalysisStatement", "")

        if pre_analysis_root_emotion != "Not identified" and pre_analysis_statement:
            system_instruction_parts.insert(
                1,
                f"From an initial analysis, the core issue for this session is identified as "
                f"'{pre_analysis_root_emotion}' due to '{pre_analysis_cause_of_emotion}'. "
                f"User's initial understanding: '{pre_analysis_statement}'. Refer to this if appropriate to keep "
                f"the Empty Chair dialogue focused. "
            )

        suggest_switch_prompt = ""
        last_n_turns = 3
        user_chat_messages = [msg.parts[0].text for msg in conversation_history if msg.role == "user"]
        blue_count = sum(1 for t in user_chat_messages[-last_n_turns:] if "[BLUE CHAIR]" in t)
        red_count = sum(1 for t in user_chat_messages[-last_n_turns:] if "[RED CHAIR]" in t)

        if blue_count == last_n_turns and perspective == 'blue':
            suggest_switch_prompt = (
                " Perhaps now, how might the person in the 'RED Chair' respond to that, or what do you imagine "
                "they would say if they heard you say that? "
            )
        elif red_count == last_n_turns and perspective == 'red':
            suggest_switch_prompt = (
                " And from your 'BLUE Chair' perspective, what does hearing 'RED Chair' say that bring up for you? "
            )

        system_instruction_parts.append(suggest_switch_prompt)
        ...
        print(f"CRITICAL CRASH calling the AI model in processMessage: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        print(f"ERROR saving messages to Firestore in processMessage: {e}")
...
def generateSessionSummaries(request):
    ...
    based on the entire Empty Chair session transcript, plus embeddings for
    each so future sessions can retrieve them via semantic search.
    """
    try:
        request_json = request.get_json(silent=True)
        session_id = request_json["sessionId"]
    ...
    try:
        ...
        for msg_doc in messages_query:
            ...
            elif role == "ai" and msg_data.get("phase") == "empty_chair_ready":
